...
The investigation converts into a behavioral search that groups network events by source, destination, destination port, and time window. A useful alert requires both rarity and context:

- the destination service is not part of the approved baseline;
- the source has not recently contacted that service;
- volume or periodicity differs from peer systems; and
- endpoint or application evidence supports further review.

The detection should not alert only because a port is rare. Notebook services, multicast discovery, health checks, and container orchestration can all create unusual but legitimate patterns.
